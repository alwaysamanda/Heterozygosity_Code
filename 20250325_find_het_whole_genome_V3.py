...
num_aut_chr = int(sys.argv[4]) # Number of autosomal chromosomes in the genome
...
def calc_het(chromosome_length_file, clade, species, num_aut_chr):
    ...
    for chr_name in chr_df.iloc[:,0]: ## Read in chromosome files
        file_path = os.path.join(clade, species, chr_name + '_het.txt')
    ...
    mean_het_file_name = os.path.join(clade, species, species + '_per_chr_mean_heterozygosity.txt')
    ...
    total_mean_file_name = os.path.join(clade, species, species + '_whole_genome_mean_heterozygosity.txt')
